...
function AjouterUser() {
  ...
  const [delegue, setDelegue] = React.useState({ value: "0", label: "Délégué"});
  ...
  function submitForm(event) {
    ...
    if (
      ...
      dispatch(userAdded({ nomU,prenomU, email, tel, login, password, id, etat, role,ligne,secteur,crm,type })).then(e=>{
        if(e.payload ===true){
      ...
      setTimeout(async () => {
        if (isNaN(location.id) === false) {
          ...
          if(entities === false){
            ...
            entities.type === 1?setDelegue({ value: "1", label: "Délégué commercial"}):setDelegue({ value: "0", label: "Délégué"})
            setId(location.id);
            resolve(entities);
          }
        } else {
  ...
  }, [location.id,dispatch,token,getLigne,getSecteur,idLine,idrole,navigate]);
                            ...
                            <Form.Group>
                              ...
                              <Select
                                ...
                                onChange={(value) => {
                                  setRoleSelect(value);
                                  setRole(value.value);
                                  if(value.value !==2)  setDelegue({ value: "0", label: "Délégué"})
                                }}
                                options={options}
                              />
                            </Form.Group>
